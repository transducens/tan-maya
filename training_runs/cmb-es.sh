...
function train_model() {
	dir=/neural/alou/tan-maya/multilingual
	source=cmb
	target=es
	if [ $source == es ]
	then
		train_pref=$dir/train/data.bpe.id
		dev_pref=$dir/dev/data.bpe.id
	else
	...
	eval "CUDA_VISIBLE_DEVICES=0,1,2,3 fairseq-train $trainArgs --seed $RANDOM --save-dir $dir/model_$source-$target/checkpoints $dir/model_$source-$target &> $dir/model_$source-$target/train.log"
}
