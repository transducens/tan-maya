...
function translate_test() {
	...
	then
		...
		CUDA_VISIBLE_DEVICES=0 fairseq-interactive --input $test_dir/data.bpe.es --path cmb/model/checkpoints/checkpoint_best.pt cmb/model | grep '^H-' | cut -f 3 > $target_dir/data.output-train
	fi
}
