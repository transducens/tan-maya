...
#        then
#            tail +1001 $vocab | tail -1000 | cut -f4 >> cmb/dev/data.cmb
#            tail +1001 $vocab | tail -1000 | cut -f5 >> cmb/dev/data.es
...
# tzh needs its own thing because it was not mined from the same sources as the other vocabs
 ...
 tail +1001 $vocab | head -1000 | cut -f2 >> cmb/dev/data.es
...
sed -i "s/\.//" cmb/test/tzh/data.tzh 
sed -i "s/\.//" cmb/test/tzh/data.es 
