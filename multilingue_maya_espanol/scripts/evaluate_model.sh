...
function translate_test() {
	...
	then
		...
		CUDA_VISIBLE_DEVICES=0,1,2,3 fairseq-interactive --input $test_dir/data.bpe.$lang --path cmb/model/checkpoints/$checkpoint cmb/model | grep '^H-' | cut -f 3 > $target_dir/data.output-train
	fi
}
